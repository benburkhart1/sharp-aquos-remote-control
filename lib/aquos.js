...
module.exports.Aquos = function(hostname, port, username, password) {
	...
	this.connect = function(callback) {
		...
		this.socket.on('close', function() {
			return callback("Premature close of connection.");
		});
	 ...
	 */
	this.channelUp = function(callback) {
	 ...
	 * @param callback callback a function with an error or success of changing the channel down
	 */
	this.channelDown = function(callback) {
		self.callback = callback;
	 ...
	 * @param callback callback a function with an error or the current power state
	 */
	this.power = function(power, callback) {
		self.callback = callback;
	 ...
	 */
	this.volume = function(level, callback) {
		if (level < 0 || level > 60)
	 ...
	 */
	this.captioning = function(level, callback) {
		self.callback = callback;
